...
async function generateNoteFiles() {
  ...
  try {
    // Read all files in public/notes/
    const files = await fs.readdir(notesDir);
    // Filter for .html files matching note-YYYYMMDDHHMMSS.html pattern
    const noteFiles = files.filter(file => /^note-\d{14}\.html$/.test(file));

    // Generate JavaScript content
    const content = `export const noteFiles = ${JSON.stringify(noteFiles, null, 2)};\n`;

    // Write to src/utils/noteFiles.js
